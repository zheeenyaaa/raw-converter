...
logging.basicConfig(filename='yandex_bot.log', level=logging.INFO, 
...
def send_welcome(message: types.Message):
    logging.info(f"Sent welcome message to {message.from_user.full_name}")
...
def process_yandex_disk_link(message):
    try:
        ...
        logging.info(f"Starting conversion for link: {public_url}")
        ...
        with tempfile.TemporaryDirectory(dir=work_dir) as temp_dir:
            ...
            logging.info(f"Download successful in archive: {input_archive}")        
            ...
            logging.info(f"Converted all in {output_dir}")
            ...
            with open(archive_path, 'rb') as archive:
                try:
                    ...
                    logging.info("Sending document successfully")
